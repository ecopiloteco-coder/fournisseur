...
const PROJECTS = [
  {
    id: 'SP-001', name: 'Rénovation Lycée Pasteur', progress: 65, color: 'primary',
    nextStep: 'Installation tableaux', deadline: '2024-04-10',
    timeline: [
  ...
  {
    id: 'SP-004', name: 'Mairie de Nanterre', progress: 15, color: 'success',
    nextStep: 'Passage de câbles', deadline: '2024-05-15',
    timeline: [
...
export function SuiviProjetPage() {
            ...
            <div className="card border-0 shadow-sm h-100">
              <div className="card-body p-4">
                ...
                <div className="d-flex gap-2">
                  <button onClick={() => { setSelectedProject(p); setShowModal(true); }}
                  ...
                  {selectedProject.timeline.map((item, index) => (
                    <div key={index} className="timeline-item position-relative ps-5 pb-4 text-start">
                      <div className={`timeline-dot position-absolute rounded-circle bg-white border border-2 border-${item.status === 'done' ? 'success' : item.status === 'current' ? 'primary' : 'secondary'} d-flex align-items-center justify-content-center`}
                        ...
                        {item.status === 'done' && <i className="fi fi-rr-check text-success" style={{ fontSize: '10px' }}></i>}
                      ...
                      <div>
                        <div className="d-flex justify-content-between align-items-center mb-1">
                          ...
                          <span className="badge bg-light text-muted fw-normal" style={{ fontSize: '0.75rem' }}>{item.date}</span>
                        </div>
